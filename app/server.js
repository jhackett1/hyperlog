...
function deleteOldFiles(){
  ...
  if (metadata.recordings.length > archiveLength) {
    ...
    fs.unlink(deletePath, function(){
      console.log("Deleting oldest recording: "+ fileName);
    })
...
function record(duration){
  ...
  var req = https.request(options, function(res) {
    ...
    res.on('end', function(){
      ...
      //
      body = JSON.parse(body).currentShow;
      if (body.currentShow == undefined) {
        var showName = "Jukebox";
      } else {
        var showName = body.currentShow[0].name;
      }
      ...
      meta.fileName = stationName + "_" + meta.txTime + "_" + meta.txDate + "_" + meta.showName + ".mp3";
